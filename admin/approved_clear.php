...
if (isset($_GET['id'])) {
    ...
    // Step 1: Kunin ang record mula sa clearance_request table
    ...
    if ($row) {
        ...
        // Step 2: Ipasok sa clearance_approved table
        $insertQuery = "INSERT INTO clearance_approved (type, name, address, birthday, year_stay_in_brgy, purpose, date, amount, username, contact, status) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $status = 'approved';
        ...
        if ($insertStmt->execute()) {
            // Step 3: Burahin ang lumang record sa clearance_request
            $deleteQuery = "DELETE FROM clearance_request WHERE id=?";
            $deleteStmt = $conn->prepare($deleteQuery);
            $deleteStmt->bind_param("i", $id);
            $deleteStmt->execute();

            // Step 4: Magpadala ng email notification
            ...
            try {
                ...
                $mail->isHTML(true);
                $mail->Subject = "Your Clearance Request is Approved";
                $mail->Body    = "
                    <h3>Hi Mr./Mrs. {$userName},</h3>
                    <p>Your clearance request has been <strong>approved</strong> and is now being processed.</p>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            Swal.fire({
                ...
                window.location.href = "clear_request.php"; // Redirect after clicking OK
            });
        });
