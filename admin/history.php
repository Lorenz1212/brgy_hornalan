...
if (isset($_GET['id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        $documentType = $row['type']; // Uri ng dokumento

        // ✅ Listahan ng documents na may 7-day cooldown
        $cooldownDocuments = [
            "Barangay Clearance",
            "Business Clearance",
        ...
        // ✅ Check kung may cooldown o wala (Tanggalin ang oras, date lang)
        $cooldown = in_array($documentType, $cooldownDocuments) ? "DATE(DATE_ADD(NOW(), INTERVAL 7 DAY))" : "NULL";

        // Ipasok sa history table kasama ang claimed_date at cooldown (DATE format)
        $historyQuery = "INSERT INTO history (type, name, address, birthday, year_stay_in_brgy, purpose, date, amount, username, contact, claimed_date, cooldown_until) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), $cooldown)";
        ...
        if ($historyStmt->execute()) {
            ...
            try {
                ...
                $mail->Body    = "
                    ...
                    <p><strong>Cooldown Period:</strong> " . (in_array($documentType, $cooldownDocuments) ? "You can request a new document after 7 days." : "No cooldown for this document.") . "</p>
                    <br>
                    <p>Best regards,</p>
                ...
                $mail->send();
                $success = true;
                $message = "Successfully moved to History! " . (in_array($documentType, $cooldownDocuments) ? "Cooldown set for 7 days." : "No cooldown applied.") . " Email notification sent.";
            } catch (Exception $e) {
                $success = true;
                $message = "Successfully moved to History! " . (in_array($documentType, $cooldownDocuments) ? "Cooldown set for 7 days." : "No cooldown applied.") . " But email failed: {$mail->ErrorInfo}";
            }
        } else {
