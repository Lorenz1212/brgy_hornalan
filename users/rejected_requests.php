...
$query = "
    SELECT id, type, name, address, birthday, year_stay_in_brgy, purpose, date, amount, username, contact, original_table, status, deleted_at, status_reason
    FROM trash 
    WHERE username = ? 
...
<div class="content-wrapper">
    ...
    <div class="table-wrapper">
        <table class="custom-table" id="rejectedRequestsTable">
        <thead>
            <tr>
                <th>Document Type</th>
                <th>Name</th>
                ...
                <th>Contact</th>
                <th>Status</th>
                <th>Status Reason</th> <!-- Added Status Reason column -->
                <th>Rejected At</th>
            </tr>
        </thead>
        <tbody>
            <?php while ($row = $result->fetch_assoc()): ?>
                <tr>
                    ...
                    <!-- Check if status_reason exists and highlight it in red -->
                    <td style="color: <?php echo !empty($row['status_reason']) ? 'red' : 'black'; ?>;">
                        <?php echo htmlspecialchars($row['status_reason']); ?>
                    </td>

                    <td><?php echo date("F d, Y", strtotime($row['deleted_at'])); ?></td> <!-- ✅ Time Removed -->
                </tr>
            <?php endwhile; ?>
        </tbody>
    </table>
...
$(document).ready(function() {
    let table = $('#rejectedRequestsTable').DataTable({
        "paging": true,
        "pageLength": 10,
        "lengthChange": false,
        "ordering": true,
