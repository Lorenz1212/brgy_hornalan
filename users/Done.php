...
if (isset($_GET['id'])) {
    ...
    // ✅ Kunin ang record mula sa clearance table
    ...
    if ($row) {
        // ✅ Ipasok sa clearance_request table kasama ang time
        $insertQuery = "INSERT INTO clearance_request 
                        (type, name, address, birthday, year_stay_in_brgy, purpose, date, amount, contact, username, time) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
        ...
        if ($insertStmt->execute()) {
            // ✅ Burahin ang record mula sa clearance table
            $deleteQuery = "DELETE FROM clearance WHERE id=?";
